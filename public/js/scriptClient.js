$(document).ready(function() {
    ...
    $('.addCompte').click(function(event) {
        ...
        $.ajax({
            url: '/mesprojets/BDP-ORM/public/ajax/compteController.php',
            type: 'GET',
            data: { getNumC: 0 },
    ...
    $('.listCompte').click(function(event) {
        var btnClick = $(event.target).closest('button');
        var idClient = btnClick.attr('idCl');
        $('#listCompteDuClient').load('/mesprojets/BDP-ORM/public/ajax/listCompte.php?idCl=' + idClient)
    })
    $('.supprimerCl').click(function(event) {
        ...
        setTimeout(function() {
            ...
            if (rep) {
                $.ajax({
                    url: '/mesprojets/BDP-ORM/public/ajax/clientController.php',
                    type: 'GET',
                    data: { del: id },
                    success: function(data) {
                        if (data != 0) {
